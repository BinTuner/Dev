...
class TuningRunMain(object):
  ...
  def main(self):
    self.init() #start tuning
    try:
      self.tuning_run.state = 'RUNNING'
      self.commit(force=True)
      List = self.search_driver.main() #Michael tuning finish
      #print "MMM"
      #print List[0][0].configuration
      #print self.search_driver.best_result.configuration
      print "----"
      if self.search_driver.best_result:
        ...
        #self.measurement_interface.save_list_config(List)     		
