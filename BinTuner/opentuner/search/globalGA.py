  ...
  def desired_configuration(self):
    ...
    for z in xrange(10): #retries
      ...
      if 'icc' in cfg:
        ...
        #print '--------Test HH------------'
        #print(copycfg)
        #if copycfg['whole-program-vtables'] == 'on':
         # cfg['lto']= 'on'
        if copycfg['incremental-linker-compatible'] == 'on':
          cfg['integrated-as']= 'on'
          #print(cfg['lto'])
        ...
        #Invoking the Z3 Prover() Xiaolei 080817   1031/2017
        optionsDic = {'ftreeCpp':copycfg['tree-ccp'],'ftreeBitCpp':copycfg['tree-bit-ccp'],'fEPathsDereference':copycfg['isolate-erroneous-paths-dereference'],'fNPointerChecks':copycfg['delete-null-pointer-checks'],'fpartialInlining':copycfg['partial-inlining'],'finlineFuntions':copycfg['inline-functions'],'fSmallFuntions':copycfg['inline-small-functions'],'findirectInlining':copycfg['indirect-inlining'], 'fTreeLoopVectorize':copycfg['tree-loop-vectorize'], 'fTreeVectorize':copycfg['tree-vectorize'], 'fShrinkWrapSeparate':copycfg['shrink-wrap-separate'], 'fShrinkWrap':copycfg['shrink-wrap'], 'fSlpVectorize':copycfg['tree-slp-vectorize']}
      
        z3first,z3second,z3third,z3fouth,z3fifth = Z3Prover.z3function(options = optionsDic)  
     
        print "[ Z3 return Results = %s %s %s %s %s]" %(z3first,z3second,z3third,z3fouth,z3fifth) 
        ...
        if z3third == False:
	    print 'random choice --->'
	    thirdRchoice = random.choice(['inline-functions','inline-small-functions'])
            cfg[thirdRchoice] = 'on'
	    print '[ Changed' ,thirdRchoice, 'value ]'
        if z3fouth == False:
            cfg['tree-vectorize'] = 'on'
            print '[ Changed "tree-vectorize" value ]'
        if z3fifth == False:
            cfg['shrink-wrap'] = 'on'
            print '[ Changed "shrink-wrap" value ]'
    ...
    for flag,value in items:
      if OptLevel == ' -O0':
         ...
         if flag == 'shrink-wrap-separate' and cfg[flag] == 'default':  
            cfg[flag] = 'on'
         if flag == 'shrink-wrap' and cfg[flag] == 'default':  
            cfg[flag] = 'off'
         ...
         if flag == 'tree-slp-vectorize' and cfg[flag] == 'default':
            cfg[flag] = 'off'
         if flag == 'indirect-inlining' and cfg[flag] == 'default':
            cfg[flag] = 'off'            
      if OptLevel == ' -O1':
         if flag == 'tree-ccp' and cfg[flag] == 'default':
            cfg[flag] = 'on'
...
class GreedySelectionMixin(object):
  ...
  def select(self):
    ...
    if (self.driver.best_result is not None and
      ...
      #print best.NCP
      return best.configuration.data
