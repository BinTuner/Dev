...
class LogDisplayPlugin(DisplayPlugin):
  def display(self, t=None):
    ...
    List = list(self.driver.results_query(objective_ordered = True))
    #print best
    #print List[0]
    if best is None:
    ...
    temp = map(_.requestor, best.desired_results)
    #Michael Display    
    #print best.time
  ...
  def display_Rank(self, t=None): #Michael NCP !!!!!!! (10/23)
    if not t:
      t = time.time()
    count = self.driver.results_query().count()
    best = self.driver.results_query(objective_ordered = True).first()
    ...
    for i in range(1, 20, 1):
      if List[i].NCP != check and count != 4:
        #print List[i].NCP		
        requestor = ','.join(map(_.requestor, List[i].desired_results))
        display_log.info("UTA, Rank "+ str(i+1) + " %s, cost %s, found by %s",
                       cfg_repr(List[i].configuration),
                       self.driver.objective.display(List[i]),
                       requestor,
                       )
        check = List[i].NCP
        count = count + 1	
  ...
  def return_List_UTA(self, t=None): #Michael
    if not t:
      t = time.time()
    count = self.driver.results_query().count()
    best = self.driver.results_query(objective_ordered = True).first()
