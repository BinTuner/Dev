...
class BanditQueue(object):
  ...
  def bandit_score(self, key):
    #print self.exploitation_term(key) #+ self.C * self.exploration_term(key)
    return (self.exploitation_term(key) +
            self.C * self.exploration_term(key))

  def ordered_keys(self): #Michael Key
    """select the next technique to use"""

    keys = list(self.keys)
    print keys
    #['DifferentialEvolutionAlt', 'UniformGreedyMutation', 'NormalGreedyMutation', 'RandomNelderMead']
    #keys = ['NormalGreedyMutation']
    #print keys
